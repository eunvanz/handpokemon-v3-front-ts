...
export default class ImageTools {
  static resize(
    file: File,
    maxDimensions: any,
    callback: (file: File | Blob | null, isBlob: boolean) => void
  ) {
  ...
  static _loadImage(image: any, file: File, callback?: any) {
    if (typeof URL === 'undefined') {
      let reader = new FileReader();
      reader.onload = function(evt) {
        image.src = evt && evt.target && evt.target.result;
        if (callback) {
          callback();
        }
      };
  ...
  static convertBlobToFile(blob: Blob, filename: string, type: any) {
    let convertedFile;
    try {
      convertedFile = new File([blob], filename, { type });
    } catch (err) {
      convertedFile = Object.assign({}, blob, {
